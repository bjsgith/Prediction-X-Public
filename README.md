...
| **Reconciliation** | Pending orders are polled each cycle — nonzero fills record actual fill price and contract count, partial fills remain tracked, zero-fill `executed` orders become `unfilled`, and `cancelled`/`expired` entries are released. P&L settles on the real all-in fill cost, not the decision-time mid. |
...
| **Guardrails** | Stale futures data is rejected before any order is placed — active-session hourly bars older than 6h, daily bars older than 96h, and off-session/reopen hourly bars older than 72h. A price frozen at the same value is rejected only during active sessions, when it appears across ≥5 observations within a 12h rolling window (spanning at least 6h). Raw edge ≥ 25pp is forced to `watch` (implausible-edge gate). Per-cycle exposure caps: 20% of account value per series, 40% portfolio-wide. Directional asymmetry cap: 3 same-direction buys per series per cycle. Orders are skipped when no valid ask is present. Settlement deposit deferral prevents double-counting of Kalshi payouts. Startup is refused without valid credentials, and the Kalshi balance is reconciled against the database at initialization. |
...
- **Edge debiasing with training cutoff.** The raw model edge is systematically biased. The agent regresses realized P&L on raw edge at the most specific segment available (`series + direction + bucket` → `series + bucket` → global → prior `(0, 0.5)`). A configurable `training_cutoff_date` restricts both the regression and the fund-manager review's evidence to post-cutoff runs, preventing pre-correction data from distorting the fitted coefficients or override decisions. The traded signal is `adjusted_edge = α + β · raw_edge`.
...
| **Safety guards** | Session-aware stale/frozen-data rejection, 25pp implausible-edge gate, per-cycle exposure caps (20% series / 40% portfolio), settlement deposit deferral |
...
- **Positions** — open contracts with order-status badges (pending / partial / filled), fill price, contract count, current Kalshi mid, unrealized P&L.
